...
namespace MinecraftClientGUI
{
    ...
    public partial class Form1 : Form
    {
        ...
        public Form1(string[] args)
        {
            ...
            logpatch = $"Logs/LOG - {DateTime.Now.ToString("MM-dd-yyyy HH-mm-ss")}.log";
                ...
                for (int i = 1; i < subs.Length; i++)
                {
                    if (subs[i].Length > 0)
                    {
                        if (subs[i].Length > 1)
                        {
                            ...
                            string txttoapp = subs[i].Substring(1, subs[i].Length - 1);
                            List<string> l = new List<string>();
                            l.Add(txttoapp);
                            File.AppendAllLines($"{logpatch}", l);
                            AppendTextBox(box_output, txttoapp, color, style);
                        }
                    }
                }
        ...
        {
            if (box_output.Text.Contains("Not connected to any server")&&checkBox1.Checked)
            {
                printstring("§4Reconnecting in 5 secounds");
                timer1.Start();
                Status.Stop();
            }
        }
